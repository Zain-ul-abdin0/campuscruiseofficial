 ...
 */
	...
	while( have_posts() ){ the_post();
		...
		$sidebar_width = tourmaster_get_option('room_general', 'room-single-sidebar-width', 20);
		$content_width = 60 - intval($sidebar_width);
		
		echo '<div class="tourmaster-container tourmaster-single-with-sidebar clearfix" >';
		echo '<div class="tourmaster-column-' . esc_attr($content_width) . ' tourmaster-single-content-wrap" >';
		
		echo tourmaster_get_room_feature_media($room_option);
		...
		if( !post_password_required() ){
			echo '<div class="tourmaster-room-page-builder tourmaster-item-rvpdlr" >';
			do_action('gdlr_core_print_page_builder');
			echo '</div>';
		}

		echo '</div>'; // tourmaster-single-content-wrap

		echo '<div class="tourmaster-column-' . esc_attr($sidebar_width) . ' tourmaster-single-sidebar-wrap" >';
		echo '<div class="tourmaster-item-pdlr" >';
		tourmaster_room_get_booking_bar();
		echo '</div>'; // tourmaster-item-pdlr
		echo '</div>'; // tourmaster-single-sidebar-wrap
	...
	tourmaster_room_single_booknow_bar();
